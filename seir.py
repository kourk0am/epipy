...
class SEIR(object):
    """ This class represents the SEIR epdidemiological model. 
    See https://en.wikipedia.org/wiki/Compartmental_models_in_epidemiology#The_SEIR_model.
    
    tldr: self.par; self.simulate(); self.result
    
    Parameters for the simulation are given in self.par dictionary. This can be passed to the SEIR object upon creation or changed later. The dictionary looks like this:
    self.par = {'N': 70e6, # total population
                        'I0' : 1, # number of initial infected individuals
                        'R' : 3.5, #reproduction number
                        'T_inc' : 5.2, # incubation time (days) - no symptoms, not infectious
                        'T_inf' : 2.9, # infectious time (days) - no symptoms, infectious
    ...
    Results are written in self.result - a dict with arrays for time, susceptible population, exposed population, infected population, removed population and the R value
